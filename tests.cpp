...
{
    BOOST_CHECK_THROW(create_frame({ 0x0, 0x0, 0x3 }), std::runtime_error);
}

BOOST_AUTO_TEST_CASE(frame_wrong_end)
{
    BOOST_CHECK_THROW(create_frame({ 0x2, 0x0, 0x0 }), std::runtime_error);
}

BOOST_AUTO_TEST_CASE(frame_valid)
{
    BOOST_CHECK_NO_THROW(create_frame({ 0x2, 0x0, 0x3 }));
}
